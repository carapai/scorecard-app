...
export default function ScorecardTable({orgUnits, nested}) {
    ...
    const {loading} = useTableOrgUnits({dataEngine, orgUnits})
    ...
    return (
        <div className="w-100 pb-32 flex-1 print-area">
            {
                loading ? <TableLoader orgUnits={orgUnits}/> : <DndProvider backend={HTML5Backend}>
                    <DataTable
                    ...
                    >
                        <TableHeader dataEngine={dataEngine} orgUnits={orgUnits} nested={nested}/>
